...
export interface DeleteTamizajesResult {
   total: number
   deleted: number
   failed: number
...
export function useDeleteTamizajesMutation() {
   const queryClient = useQueryClient()

   return useMutation({
      mutationFn: async (ids: string[]): Promise<DeleteTamizajesResult> => {
         const errors: { id: string; message: string }[] = []
         let deleted = 0

         for (const id of ids) {
            try {
               await deleteTamizaje(id)
               deleted++
            } catch (error) {
               errors.push({
                  id,
